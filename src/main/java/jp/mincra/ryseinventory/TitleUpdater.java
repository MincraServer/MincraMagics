...
public final class TitleUpdater {
    ...
    public static void updateInventory(Player player, String newTitle) {
        ...
        try {
            ...
            Object activeContainer = jp.mincra.ryseinventory.TitleUpdater.activeContainer.invoke(entityPlayer);

            // Get windowId from activeContainer.
            Integer windowId = (Integer) jp.mincra.ryseinventory.TitleUpdater.windowId.invoke(activeContainer);
            ...
            jp.mincra.ryseinventory.TitleUpdater.Containers container = jp.mincra.ryseinventory.TitleUpdater.Containers.getType(type, size);
            ...
            if (!useContainers() && container == jp.mincra.ryseinventory.TitleUpdater.Containers.GENERIC_3X3) {
    ...
    private static MethodHandle getConstructor(Class<?>... types) {
        try {
            Constructor<?> constructor = jp.mincra.ryseinventory.TitleUpdater.PACKET_PLAY_OUT_OPEN_WINDOW.getDeclaredConstructor(types);
    ...
    private enum Containers {
         ...
         */
        public static jp.mincra.ryseinventory.TitleUpdater.Containers getType(InventoryType type, int size) {
            if (type == InventoryType.CHEST) {
                return jp.mincra.ryseinventory.TitleUpdater.Containers.valueOf("GENERIC_9X" + size / 9);
            }
            for (jp.mincra.ryseinventory.TitleUpdater.Containers container : jp.mincra.ryseinventory.TitleUpdater.Containers.values()) {
