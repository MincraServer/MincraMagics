...
public class MpHudController {
    private static final List<String> noMpIconCode = List.of(
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_zero_1%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_zero_2%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_zero_3%")
    );
    private static final List<String> halfMpIconCode = List.of(
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_half_1%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_half_2%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_zero_3%")
    );
    private static final List<String> fullMpIconCode = List.of(
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_full_1%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_full_2%"),
            PlaceholderAPI.setPlaceholders(null, "%oraxen_hud_mana_zero_3%")
    );
    ...
    public String generateMpBar(int maxMp, int currentMp) {
        ...
        for (int i = line - 1; i >= 0; i--) {
            int maxMpForLine = Math.min(20, Math.max(0, maxMp - i * 20)); // 15
            int currentMpForLine = Math.min(20, Math.max(0, currentMp - i * 20)); // 11
            int mpDiffForLine = maxMpForLine - currentMpForLine; // 4

            int noMpLen = (mpDiffForLine + 1) / 2; // 2
            int halfMpLen = (currentMpForLine & 1); //奇数なら1 偶数なら0 // 1
            int fullMpLen = currentMpForLine / 2; // 5

            result.append(noMpIconCode.get(i).repeat(Math.max(0, noMpLen)))
                  .append(halfMpIconCode.get(i).repeat(halfMpLen))
                  .append(fullMpIconCode.get(i).repeat(fullMpLen));
